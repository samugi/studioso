...
def test_prompt_config_loads_frontmatter_and_behavior():
    ...
    assert config.quiz_labels["correct"] == "correct"
    assert config.output_labels["question"] == "QUESTION"
    assert "## Role" in config.behavior
...
def test_prompt_config_renders_templates():
    ...
    assert rendered.startswith("User question")
