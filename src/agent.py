...
class StudyAgent:
    ...
    def check_ollama(self) -> tuple[bool, str]:
        ...
        try:
            ...
            def normalize(name): return name.split(":")[0]
    ...
    def _build_reference_mode_system_prompt(self, context_chunks: list[dict], mode: str = "qa") -> str:
        # Format retrieved context
        context_parts = []
        for i, chunk in enumerate(context_chunks, 1):
            excerpt = chunk["text"][:self.source_excerpt_length * 2]  # full text for model
...
"""
    ...
    def answer_question(self, question: str, context_chunks: list[dict], history: list[dict] = None) -> str:
    ...
    def answer_question_stream(self, question: str, context_chunks: list[dict], history: list[dict] = None) -> Iterator[str]:
    ...
    def generate_question(self, context_chunks: list[dict], previous_questions: list[str] = None) -> str:
        ...
        messages = []
        messages.append({"role": "system", "content" : system})
        messages.append({"role": "user", "content": f"Genera UNA domanda da quiz dal materiale di studio.{avoid}"})
    ...
    def evaluate_answer(self, question: str, user_answer: str, context_chunks: list[dict]) -> str:
        """Valuta la risposta dell'utente basandoti sul materiale di studio fornito."""
        system = self._build_reference_mode_system_prompt(context_chunks, mode="quiz-evaluation")
...
Sii incoraggiante ma preciso. Non considerare risposte vaghe o incomplete come completamente corrette."""
    ...
    def evaluate_answer_stream(self, question: str, user_answer: str, context_chunks: list[dict]) -> Iterator[str]:
